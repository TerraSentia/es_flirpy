...
class Boson(Core):
    ...
    def find_video_device(self):
        ...
        if sys.platform.startswith("win32"):
            ...
            udev = pyudev.Devices.from_path(context, video_devices[0])
            dev.append(int(video_devices[0].split("video")[-1]))

            # for device in video_devices:
            #     udev = pyudev.Devices.from_path(context, device)

            #     try:
            #         vid = udev.properties["idVendor"]
            #         pid = udev.properties["idProduct"]

            #         if vid.lower() == "09cb" and pid.lower() == "4007":
            #             dev.append(int(device.split("video")[-1]))
            #     except KeyError:
            #         raise ValueError("udev error maybe.")
